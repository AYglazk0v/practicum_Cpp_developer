...
class SearchServer {
    ...
    public:
        ...
        requires std::is_same_v<std::decay_t<ExecutionPolicy>, std::execution::sequenced_policy>
        std::vector<Document> FindAllDocuments(ExecutionPolicy&& policy, const Query<>& query, DocumentPredicate document_predicate) const;

        template <typename DocumentPredicate, typename ExecutionPolicy>
        requires std::is_same_v<std::decay_t<ExecutionPolicy>, std::execution::parallel_policy>
        std::vector<Document> FindAllDocuments(ExecutionPolicy&& policy, const Query<>& query, DocumentPredicate document_predicate) const;
...
std::vector<Document> SearchServer::FindAllDocuments(ExecutionPolicy&& policy, const Query<>& query, DocumentPredicate document_predicate) const {
    ...
    auto map_reduce = document_to_relevance.BuildOrdinaryMap();
    std::vector<Document> matched_documents;
    matched_documents.reserve(map_reduce.size());
    for (const auto [document_id, relevance] : map_reduce) {
        matched_documents.push_back({document_id, relevance, documents_.at(document_id).rating});
    }
    return matched_documents;
}
