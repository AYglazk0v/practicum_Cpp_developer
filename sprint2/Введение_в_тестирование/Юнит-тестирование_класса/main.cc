...
public:
    ...
    bool AreSynonyms(const string& first_word, const string& second_word) const {
        if (synonyms_.count(first_word) == 1 && synonyms_.at(first_word).count(second_word) == 1) {
            return true;
        }
        return false;
    }
...
void TestAreSynonyms() {
    ...
    assert(synonyms.AreSynonyms("cat"s, "kitty"s) == false);

    synonyms.Add("cat"s, "kitty"s);
    assert(synonyms.AreSynonyms("cat"s, "kitty"s) == true);
    assert(synonyms.AreSynonyms("kitty"s, "cat"s) == true);
}
