...
public:
    ...
    bool operator==(VehiclePlate& rhs) const {
        return letters_ == rhs.letters_
                && digits_ == digits_
                && rhs.region_ == rhs.region_;
    }
...
public:
    void Insert(T elem) {
        ...
        if (auto it = std::find(elements_[index].begin(), elements_[index].end(), elem); it == elements_[index].end()) {
            elements_[index].push_back(std::move(elem));
        }
    }

    void PrintAll(ostream& out) const {
        for (auto& e : elements_) {
            for (auto& curr_elem : e) {
                out << curr_elem << std::endl;
            }
        }
    }
