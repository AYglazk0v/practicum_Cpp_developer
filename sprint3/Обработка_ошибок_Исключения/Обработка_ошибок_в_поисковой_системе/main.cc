...
public:
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status,
        ...
        if ((document_id < 0)) {
            throw std::runtime_error("Bad ID: "s + std::to_string(document_id));
        }
        if (documents_.count(document_id) == 1) {
            throw std::runtime_error("Bad ID: "s + std::to_string(document_id) + " already exists!");
        }
        vector<string> words;
        if (!SplitIntoWordsNoStop(document, words)) {
            throw std::runtime_error("Error in split into words no stop");
        }
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate) const {
        ...
        if (!ParseQuery(raw_query, query)) {
            throw std::runtime_error("Bad Query: "s + raw_query);
        }
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
        ...
        Query query;
        if (!ParseQuery(raw_query, query)) {
            throw std::runtime_error("Bad pars query in MatchDocument");
        }
