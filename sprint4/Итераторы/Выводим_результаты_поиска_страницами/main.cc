...
std::ostream& operator<<(std::ostream& os, const std::vector<Document>& docs) {
...
class Paginator {
    public:
        Paginator(Iterator it, Iterator ite, size_t page_size) {
            std::vector<std::vector<typename Iterator::value_type>> tmp_pages(std::distance(it, ite) / page_size + 1, std::vector<typename Iterator::value_type>(page_size));
