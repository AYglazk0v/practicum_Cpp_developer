...
void SortBy(Container& container, KeyMapper key_mapper, bool reverse = false) {
    if (reverse){
        std::sort(container.rbegin(), container.rend(), [&key_mapper](const auto& lhs, const auto& rhs){return key_mapper(lhs) < key_mapper(rhs);});
    } else {
        std::sort(container.begin(), container.end(), [&key_mapper](const auto& lhs, const auto& rhs){return key_mapper(lhs) < key_mapper(rhs);});
    }
}
