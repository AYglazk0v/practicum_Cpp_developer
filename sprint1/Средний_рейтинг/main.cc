...
public:
		...
		vector<Document> FindTopDocuments(const string& query) const {
				...
				for (auto& curr_doc : matched_documents) {
					curr_doc.rating = document_id_to_average_raiting_.at(curr_doc.id);
				}
...
private:
		...
		map<int,int>					document_id_to_average_raiting_;
		...
		vector<Document> FindAllDocuments(const string& query) const {
				...
				for (const string& word : query_words.word_plus_) {
						...
						for (auto [id, freq]: word_to_documents_freq_.at(word)) {
							document_to_relevance[id] += freq * std::log((document_id_to_average_raiting_.size() + 0.0)/ word_to_documents_freq_.at(word).size());
						}
				}
