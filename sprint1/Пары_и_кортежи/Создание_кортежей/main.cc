...
public:
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
        ...
        for (const auto& curr_word : query.plus_words) {
            if (word_to_document_freqs_.count(curr_word) == 0) {
                continue;
            }
            if (word_to_document_freqs_.at(curr_word).count(document_id)) {
                matched_words.push_back(curr_word);
            }
        }
        for (const auto& curr_word : query.minus_words) {
            if (word_to_document_freqs_.count(curr_word) == 0) {
                continue;
            }
            if (word_to_document_freqs_.at(curr_word).count(document_id)) {
